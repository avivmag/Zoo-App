...
namespace DAL
{
    ...
    public partial class NegevZooDBEntities : IZooDB
    {
        ...
        {
            var tableKey = typeof(T).FullName;

            var cacheItem = NegevZooDBEntities.cache.Get<List<T>>(tableKey);
            
            if (cacheItem == null)
            {
                var tableData = table.AsNoTracking().ToList();

                NegevZooDBEntities.cache.Set<List<T>>(tableData,  tableKey);
        ...
        {
            NegevZooDBEntities.cache.Remove(key);
        }
